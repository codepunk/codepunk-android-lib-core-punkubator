...
class MaxLengthValidatinator : AbsValidatinator<CharSequence?> {
    ...
    constructor(max: Int = Integer.MAX_VALUE, message: CharSequence?) : super(message) {
        this.max = Math.max(max, 0)
    }
    ...
    constructor(
        ...
        getMessage: (input: CharSequence?) -> CharSequence? = {
            val tMax = Math.max(max, 0)
            context.getString(R.string.validatinator_invalid_max, valueName, tMax)
        }
